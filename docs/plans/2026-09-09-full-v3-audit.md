...
- Freeze explicit wire identifiers and field semantics. V3 on-disk structures use Serde/Postcard, including declaration-order enums for actions and crypto profiles (`logfs/src/journal/data.rs`, `logfs/src/crypto.rs`). Reordering variants or fields can silently alter the format even when the Rust change looks harmless. Released v2 data retains its legacy bincode decoder.
- Define required versus optional features and unknown-record handling. The completed specification now requires version 3 exactly, rejects unknown action variants and trailing Postcard payload bytes, and reserves schema changes for a new format version rather than ad hoc extensions.
...
The findings above are preserved as the original audit, including their historical paths and line numbers. The implementation now lives in `logfs/src/journal/v3`; the old `v2` module retains legacy decoding/checkpoint logic and compatibility re-exports. Shared action types live directly under `journal`, with version-specific codecs. Public API names, including `Journal2`, remain compatible. The deliberate pre-release switch from bincode to Postcard changed v3 inner payload bytes; key derivation contexts, nonce schedules, and normal-open history checks did not change.
...
- **Finding 4:** Shared v3 action/checkpoint plaintext limits are 512 MiB; encoded limits separately include authentication overhead. Writer measurement precedes serialization, read-buffer limits precede allocation, and decompressed checkpoint input remains bounded by committed region length and the implementation budget. The Postcard snapshot serializer borrows the key tree, measures it before allocation, and preserves the owned snapshot layout.
- **Organization:** Dedicated v3 `codec`, `format`, `root`, `index`, `limits`, `read`, `write`, and `repair` modules. Legacy root/entry decoding and parented checkpoint restoration are in `v2/codec.rs` and `v2/index.rs`; the v3 writer rejects legacy writes. Read adapters retain explicit version dispatch for the public API.
...
- **Specification:** Added [`docs/v3-format.md`](../v3-format.md), covering Postcard byte layouts and enum ordinals, coordinates, envelopes, KDF/AEAD domains, framing, snapshots, commit/recovery policy, resource limits, and evolution rules. The distinction between strict replay, salvage, and metadata-only dry-run is explicit.
